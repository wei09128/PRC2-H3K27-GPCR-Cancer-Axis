...
class BalancedCellTypeAnnotator:
    ...
    def annotate_balanced(self, adata, sample_col: str = 'Sample_ID', 
        ...
        1. Marker scores (60%): Primary evidence from gene expression
        2. Sample metadata (30%): Cancer type prior from sample
        3. Cluster consistency (10%): Spatial smoothing via UMAP clusters
        
        Strategy:
        - For cancer cells: Sample metadata provides strong prior
        - For immune/stromal: Rely more on markers (sample-agnostic)
        - UMAP clusters enforce local consistency
        """
        print(f"\n{'='*70}")
        ...
        # ==== STEP 2: Sample Cancer Type Prior ====
        print(f"2. Incorporating sample metadata...")
        sample_probs = np.zeros_like(score_matrix)
        ...
        if sample_col in adata.obs.columns:
            for i, cell_sample in enumerate(adata.obs[sample_col]):
                if cell_sample in self.sample_cancer_map:
                    ...
                    # Map metadata cancer type to marker cell types
                    if sample_cancer in cancer_types:
                        # Strong prior for matching cancer type
                        cancer_idx = cell_type_names.index(sample_cancer)
                        sample_probs[i, cancer_idx] = 1.0
                    elif sample_cancer == 'Normal Breast Cells':
                        # Could be normal epithelial or any stromal/immune
                        # Give slight boost to non-cancer types
                        for j, ct in enumerate(cell_type_names):
                            if ct not in cancer_types:
                                sample_probs[i, j] = 0.1
                    else:
                        # Unknown cancer type - no prior
                        sample_probs[i, :] = 1.0 / len(cell_type_names)
                else:
                    # No sample info - uniform prior
                    sample_probs[i, :] = 1.0 / len(cell_type_names)
            
            # Normalize
            sample_probs = sample_probs / (sample_probs.sum(axis=1, keepdims=True) + 1e-10)
        else:
        ...
        print(f"\n4. Combining evidence sources...")
        combined_probs = (marker_weight * marker_probs + 
                         sample_weight * sample_probs + 
                         cluster_weight * cluster_probs)
        
        # Normalize
        combined_probs = combined_probs / (combined_probs.sum(axis=1, keepdims=True) + 1e-10)
        ...
        if sample_col in adata.obs.columns:
            print(f"\nSample-level summary:")
            for sample in sorted(adata.obs[sample_col].unique())[:10]:
