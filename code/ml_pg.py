...
def plot_selected_features_heatmap(
    ...
    plt.xlabel("Model")
    plt.title(f"Selected Features – Pan-gyn + H3K27 Model") # PRC2/H3K27 Model
    plt.tight_layout()
    plt.savefig(out_path, dpi=300)
    plt.close()
    print(f"Saved feature selection heatmap: {out_path}")
...
def plot_all_models_cv_roc(best_models, X, y, cv, out_path="roc_all_models.png"):
    ...
    plt.xlabel("False Positive Rate")
    plt.ylabel("True Positive Rate")
    plt.title("ROC Curves (CV) – Pan-gyn + H3K27 Model")
    plt.legend(loc="lower right")
    plt.tight_layout()
    plt.savefig(out_path, dpi=300)
    plt.close()
...
def plot_nested_f1_bar(nested_results, out_path="nested_f1_bar.png"):
    ...
    plt.title("Nested CV F1 – Pan-gyn + H3K27 Model")
    plt.tight_layout()
    plt.savefig(out_path, dpi=300)
    plt.close()
