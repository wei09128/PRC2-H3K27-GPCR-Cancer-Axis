...
def build_peak_to_gene_from_gtf(
    ...
    # Decide where to get peak coordinates from:
    if peak_coord_col is not None and peak_coord_col in atac_adata.var.columns:
        peak_strings = atac_adata.var[peak_coord_col].astype(str)
        print(f"Using atac_adata.var['{peak_coord_col}'] for peak coordinates.")
    else:
        peak_strings = atac_adata.var_names.astype(str)
        print("Using atac_adata.var_names for peak coordinates.")

    # Convert to Series so split returns a DataFrame, not a MultiIndex
    peak_df = (
        peak_strings
        .to_series()
    ...
    if peak_df.shape[1] != 3:
        raise ValueError(
            f"Expected 3 columns from splitting peaks (chrom, start, end) but got shape {peak_df.shape}. "
            "Check that peak names look like 'chr1:100-200'."
        )

    # IMPORTANT: use the same names your PyRanges call expects
    peak_df.columns = ["Chromosome", "Start", "End"]
    peak_df["Start"] = peak_df["Start"].astype(int)
    peak_df["End"]   = peak_df["End"].astype(int)

    # Preserve the peak IDs
    peak_df["Peak_ID"] = atac_adata.var_names.astype(str).values

    # Optional: reorder columns (not strictly required, but nice)
    peak_df = peak_df[["Peak_ID", "Chromosome", "Start", "End"]]

    # Now this line in your original code will work:
